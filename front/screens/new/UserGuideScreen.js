...
## Целевой сценарий работы системы с нуля
1. Регистрируется пользователь с ролью "Сотрудник склада"
2. Регистрируется пользователь с ролью "Специалист по контролю качества"
3. Регистрируется пользователь с ролью "Рабочий конвейер (техническая роль)"
4. Регистрируется N пользователей с ролью "Авиаинженер"
5. Сотрудник склада создает воздушное судно для проведения обслуживания
6. Сотрудник склада добавляет необходимые инструменты в реестр инструментов
7. Сотрудник склада формирует тип набора из инструментов в реестре
8. Сотрудник склада создает фактические наборы с партийными номерами на основе типа
9. Сотрудник склада создает заявки на техническое обслуживание воздушных судов 
(с указанием воздушного судна, исполнителя с ролью "Авиаинженер", фактического набора инструментов)
10. Происходит авторизация под ролью "Рабочий конвейер (техническая роль)" в начале приемки инструментов
11. Рабочие по очереди вводят свой табельный номер. При этом подтягивается активная заявка на тех. обслуживание,
в которой рабочий назначен исполнителем
12. Далее возможно несколько вариантов фиксации инструментов:
   - Рабочий выкладывает инструменты под камерой
   - Загружается фото инструментов
   - Загружается архив фото инструментов
13. По результатам распознавания:
   - Если найдена недостача (не все инструменты найдены или не пройден порог по вероятности), 
   то создается инцидент
   - Если все инструменты найдены, то заявка на тех. обслуживание закрывается со статусом "ЗАВЕРШЕНА"
   - Следующий рабочий в очереди может вводить свой табельный номер
14. Если создан инцидент недостачи:
   - Инцидент поступает пользователю с ролью "Специалист по контролю качества"
   - Специалист берет инцидент в работу и занимется его расследованием
   - По результатми расследования формируется комментарий специалиста 
   - Инцидент считается завершенным
`
